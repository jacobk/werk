...
var createGithubAuth = function() {
  return new RSVP.Promise(function(resolve, reject) {
    ...
    if (!config.isGlobal()) {
      logger.log.writelns('Use --global write to global config');
    }
...
var hasCredentials = function() {
  ...
  logger.verbose.write('Checking config for GitHub credentials...');
  logger.verbose.ok();
  logger.verbose.ok('user: ' + user);
  logger.verbose.ok('token: ' + token);
  return token && user;
};

exports.withGitHubAuth = function() {
  return new RSVP.Promise(function(resolve, reject) {
    if (hasCredentials() && !config.forceAuth()) {
      github.authenticate({
        type: 'oauth',
        token: config.get('github_token')
      });
      logger.verbose.ok('Authenticated with GitHub as \'' +
                        config.get('github_user') + '\'');
      resolve(true);
    } else {
      if (config.forceAuth()) {
        logger.log.writelns('Forcing re-authentication with GitHub');
      }
...
exports.openPullRequest = function(repoInfo, pullRequest) {
  return new RSVP.Promise(function(resolve, reject) {
    var options = {
      repo: repoInfo.name,
      user: repoInfo.owner,
      ...
      head: pullRequest.branchName
    };
    github.pullRequests.create(options, function(err, res) {
      if (!err) {
        logger.verbose.ok();
        resolve(res);
      } else {
        reject({
          reason: 'Failed to create pull requests on GitHub',
          help: err.message
        });
      }
    });
